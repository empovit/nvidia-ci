...
def test_single_version_multiple_ocp():
    ...
    results = check_gpu_operator_availability(
        gpu_versions=["24.6.2"],
    ...
    print("\nResults:")
    gpu_ver = "24.6.2"
    available_count = sum(1 for avail in results[gpu_ver].values() if avail)
    total_count = len(results[gpu_ver])

    for ocp, available in sorted(results[gpu_ver].items()):
...
def test_multiple_versions_multiple_ocp():
    ...
    print("\nResults:")
    all_available = True
    for gpu_ver in sorted(results.keys()):
        available_count = sum(1 for avail in results[gpu_ver].values() if avail)
        total_count = len(results[gpu_ver])
...
def test_nonexistent_version():
    ...
    results = check_gpu_operator_availability(
        gpu_versions=["99.99.99"],
        ocp_versions=["4.20"]
    )

    gpu_ver = "99.99.99"
    found = any(results[gpu_ver].values())
...
def test_mixed_availability():
    ...
    results = check_gpu_operator_availability(
        gpu_versions=["25.10.1"],
        ocp_versions=["4.20", "4.19", "4.16", "4.13"]
    )

    print("\nResults:")
    gpu_ver = "25.10.1"
    for ocp, available in sorted(results[gpu_ver].items(), reverse=True):
        status = "✓" if available else "✗"
        print(f"  OCP {ocp}: {status}")
